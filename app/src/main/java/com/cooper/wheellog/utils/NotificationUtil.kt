...
class NotificationUtil(private val context: Context) {
    ...
    private fun build(): Notification {
        ...
        val buttonSettings =
            buttonsSettingsString?.split(MultiSelectPreference.separator)?.toTypedArray()
                ?: arrayOf(
                    context.getString(R.string.icon_connection),
                    context.getString(R.string.icon_logging),
                    context.getString(R.string.icon_beep),
                    context.getString(R.string.icon_light),
                    context.getString(R.string.icon_miband)
                )
        ...
        notificationView.setImageViewResource(
            R.id.ib_beep,
            WheelLog.ThemeManager.getDrawableId(R.drawable.ic_horn_32_gray)
        )
        notificationView.setImageViewResource(
            R.id.ib_light,
            WheelLog.ThemeManager.getDrawableId(R.drawable.ic_sun_32_gray)
        )
        ...
        when (WheelLog.AppConfig.mibandMode) {
            ...
            MiBandEnum.Min -> builder.setContentText(
                context.getString(
            ...
            MiBandEnum.Medium -> builder.setContentText(
                context.getString(
                    R.string.notification_text_med,
                    speed,
                    wd.averageSpeedDouble,
                    wd.calculatedPwm,
                    wd.batteryLevel,
                    wd.temperature,
                    wd.distanceDouble
            ...
            MiBandEnum.Max -> builder.setContentText(
                context.getString(
...
// Try for RS fix
        Timer().scheduleAtFixedRate(object : TimerTask() {
            override fun run() {
                if (WheelLog.AppConfig.mibandMode != MiBandEnum.Alarm && WheelData.getInstance().speed != 0) {
                    update()
                }
            }
        }, 1000, 1000)
